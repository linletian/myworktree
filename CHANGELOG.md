...
- **fix(ui): web panels no longer stack, reasonix iframe back to single-frame model** — switching between opencode-web and reasonix tabs left BOTH panels visible (`deactivate` hid only the iframe), stacking the two embeds into top/bottom halves; `selectInstance` now hides both panels before dispatching and each renderer hides its panel on deactivate. The Reasonix renderer was also rebuilt on the proven v0.4.0 model: ONE shared iframe that re-navigates on instance switch (instead of a per-instance iframe cache keeping several reasonix SPA pages alive under one origin, which blanked subsequent instances) and that only navigates while the record is `running`/`starting` (the serve is already listening by then — `Driver.Start` waits for the port), with hide-not-destroy keep-alive (issue #53), invalidate-on-stop, and the issue #54/#55 xterm cleanups intact.
