...
- **opencode-web 实例类型**（实施中；详见 `docs/plans/opencode-native-ui/PLAN.md`；威胁模型与评审检查表见 `docs/ARCHITECTURE.md` §8）—— 跳过 PTY/xterm.js，直接内嵌 opencode 官方 web UI。每个 opencode-web instance = 1 个独立的 `opencode serve` 进程（命令、`--hostname 127.0.0.1`、`--port 0` 由 myworktree 硬编码；`OPENCODE_SERVER_PASSWORD = cfg.AuthToken`——统一认证 token，全部实例共享，**不允许关闭**）。非安全 env 通过 `tag.Env` 合并。浏览器通过 `/__opencode/<id>/<base64(worktree)>/session/` iframe 加载 opencode 聊天 session 页面（反向代理路径 `/__opencode/<id>/*`，myworktree bearer token 校验 → Go 端注入 Basic auth）。前端按 instance kind 分支：PTY 仍显示 xterm.js，opencode-web 显示 iframe。多 session 切换复用 myworktree instance tab。
