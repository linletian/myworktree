...
- 规划新增：**opencode-web 实例类型**（实施中，详见 `docs/plans/opencode-native-ui/PLAN.md`）—— 跳过 PTY/xterm.js，直接内嵌 opencode 官方 web UI。每个 opencode-web instance = 1 个独立的 `opencode serve` 进程（命令、`--hostname 127.0.0.1`、`--port 0` 由 myworktree 硬编码；`OPENCODE_SERVER_PASSWORD` 每次启动由 myworktree 用 `crypto/rand` 生成并强制注入，不允许用户关闭）。非安全 env 通过 `tag.Env` 合并。浏览器通过 `/__opencode/<id>/*` 反向代理访问（myworktree 全局 Token 认证 + Go 端注入 Basic auth）。前端按 instance kind 分支：PTY 仍显示 xterm.js，opencode-web 显示 iframe。多 session 切换复用 myworktree instance tab。
