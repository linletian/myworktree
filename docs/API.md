...
`password_set` is always `true` (password is generated by myworktree and injected server-side; the frontend does not need it). `host` and `port` are the opencode server's bound address. `iframe_src` is the path to load in the iframe.
